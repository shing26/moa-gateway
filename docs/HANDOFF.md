...
- [x] 处理未提交改动（commit/stash，问用户）
- [x] `git checkout -b feat/resume-v2`
- [x] 修 `app/agents/provider.py` 空 env 崩溃（见 §3）
- [x] `pytest tests/unit -q` 全绿
- [x] commit：`fix: tolerate empty env vars in LLMConfig.from_env`

### Phase 1：换内核 LiteLLM + fallback + 成本（1-2 天）
- [x] `uv add litellm`
- [x] `app/agents/provider.py`：保留 `LLMClient` 接口（`chat`/`chat_with_tools`/`aclose`/`__aenter__`/`__aexit__`），内部换 `litellm.acompletion`；加 `fallback_models` 列表与失败重试；加 `cost_usd`/`latency_ms`/`model_used` 记录（`litellm.cost_per_token` 或 response 自带 cost）
- [x] `app/audit/models.py` + `app/middleware/request_logger.py`：`extra` 增加 `llm_model`/`cost_usd`/`llm_latency_ms`/`fallback_used`（默认值，不破坏现有调用）
- [x] 新增 `tests/unit/test_provider_litellm.py`：monkeypatch `litellm.acompletion` 验证参数透传、fallback 切换、cost 计算、空 env 不崩
- [x] 全量测试 + commit：`feat(llm): swap to litellm with fallback and cost tracking`
- 注意：现有测试大多注入 fake LLM，不受影响；**禁止让测试打真实网络**

### Phase 2：Eval 评估体系（3-4 天）★核心卖点
- [x] 建 `evals/datasets/intent.jsonl`（50 条）、`guard_redteam.jsonl`（50 条）、`e2e.jsonl`（30 条）
  - schema 见 `docs/RESUME_V2_PLAN.md` §6.2
  - **至少 5 条手写边界用例**（如"今天天气怎么样"→assistant、多意图、含内网 IP 的对抗文本）
- [x] 建 `evals/run_evals.py`：
...
- [x] 新增 `tests/unit/test_evals_runner.py`：小 fixture 数据验证指标计算
- [x] `uv run python evals/run_evals.py --offline` 出报告；intent 准确率 ≥0.9、guard deny 召回 ≥0.95（否则调数据集，**不许调阈值凑数**）
- [x] commit：`feat(evals): add intent/guard/e2e evaluation harness`

### Phase 3：工程化（1-2 天）
- [x] `.github/workflows/ci.yml`：加 `ruff check .`、`bandit -r app -q`、`python evals/run_evals.py --offline`
- [x] 检查并修 `Dockerfile`（python:3.12 + uv 安装可用）
- [x] lint 存量告警：能修的顺手修（小 diff），不能修的加 `# noqa` 或 ruff ignore 配置，**别大改码风**
- [x] commit：`chore(ci): extend CI with lint, bandit, eval smoke`

### Phase 4：垂直场景接入（1-2 天）
- [x] `app/command_mode.py` 加 `/review` 命令（intent: `review`）
- [x] 新增 `app/agents/review_agent.py`：包装 `CodeReviewPipeline`，输入 `owner/repo#PR号`，输出审查摘要文本；无 GITHUB_TOKEN 等配置时返回优雅降级文案
- [x] 注册进 `app/agents/loader.py`（`get_agent('review')`）
- [x] 新增 `tests/unit/test_review_agent.py`：注入 fake github client（参考 `tests/unit/test_github_webhook_e2e.py` 的 fixture）
- [x] commit：`feat(review): expose PR review as a gateway agent command`
- 风险提醒：`CodeReviewPipeline` 用 `GitHubClient.from_env()`，测试必须注入 fake，禁止真实网络

### Phase 5：包装（1-2 天）
- [x] `README.md` 重写：标题 `agent-gateway`、mermaid 架构图、三卖点章节（路由省钱数据 / 守卫+HITL redteam 100% / Eval 报告截图）、垂直应用章节、快速开始（含 eval 命令）、"为什么不用 LangChain/LangGraph"选型说明
- [x] `app/main.py` title 改 `Agent Gateway`；Dashboard 标题文案同步
- [ ] `pyproject.toml` name 改 `agent-gateway`（已决定：仅展示层改名，包名保持 `moa-gateway`）
- [ ] demo：录屏 + 部署链接（已决定：暂不提供公网链接，README 给本地启动/演示命令）
- [x] commit：`docs: rewrite README for resume-v2 positioning`
...
- [x] CI 全绿（pytest + ruff + bandit + eval offline）
- [x] `evals/run_evals.py --offline` 出报告，指标真实
- [x] LiteLLM 替换后接口兼容，fallback/cost 有测试
- [x] README 重写完成（架构图 + 三卖点 + 数据表 + 选型说明 + demo）
- [x] PR 审查演示路径可用（fixture 测试通过）
- [x] 工作区干净：无 `.env`/`logs/`/`data/` 提交
- [x] 所有 commit 在 `feat/resume-v2`，提交信息清晰
- [x] 向用户交付：改动摘要 + 测试结果 + README + 本地演示命令 + 简历条目草稿（公网 demo 链接按约定暂不提供）

## 9. 交接检查清单（给接手的 harness）

- [x] 已读 `docs/HANDOFF.md`、`docs/RESUME_V2_PLAN.md`、`README.md`
- [x] 已确认测试基线（362/1/9）
- [x] 已与用户确认未提交改动的处理方式
- [x] 已建 `feat/resume-v2` 分支
- [x] 按 Phase 0→5 顺序执行，每 Phase 验收后 commit
